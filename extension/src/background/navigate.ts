...
export async function navigate({ session, signal, moveSession }: ToolRuntime, args: NavigateArgs): Promise<ToolSuccessOutput> {
  ...
  if (args.action === "close_tab") {
    if (tabId === session.tabId) throw new Error("この会話を実行中のタブは閉じられません。");
...
export async function resolveTab(ref: string): Promise<number> {
  ...
  try {
    ...
    throw new Error(`タブ参照が無効です: ${ref}。list(type=tab)で参照を再取得してください。`);
  }
  return tabId;
}
...
function waitForLoad(tabId: number, signal: AbortSignal): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => finish(new DOMException("回答の生成を中止しました。", "AbortError"));
    const timeout = setTimeout(() => finish(new Error("ページ遷移後の読み込みを30秒以内に確認できませんでした。")), LOAD_TIMEOUT_MS);
